...
1. **输入素材**：原曲提供旋律、节奏和时间轴；SVS 新歌词替换原演唱内容；目标音色样本决定最终“谁来唱”。目标音色留空时沿用从原曲自动选择的音色。SVC 只换音色并保留原词。
2. **处理素材**：创建工程后依次复制原曲、拆分人声/伴奏、提取连续音高、按换气分段、识别音符并对齐新歌词。最长片段限制单次模型输入，不会截短整曲。建议默认 28 秒 / 0.3 秒；SVS 连续长句可试 40 秒，SVC 必须小于 30 秒。
3. **检查中间产物**：可直接查看分离后纯人声、伴奏、实际采用的目标音色片段和导入的乐谱文件。逐段试听原人声，检查切点、新歌词、音符和 F0 提示。
4. **生成最终人声**：先生成当前片段试听，再运行所有未完成片段。每段成功立即落盘；失败保留已完成进度。换歌词默认使用“乐谱锁定节奏”，保留原词时可比较“跟随原唱连续旋律”。
5. **导出最终产物**：草稿允许待复核片段；正式成品要求所有有声片段均已试听验收。可分别下载纯人声和混入原伴奏的整曲，拼接保持原时间轴上的前奏、间奏和尾奏。

### 逐字音符对齐（通俗模式）

工作台不再要求记忆 `note_type` 数字：

- **唱新字**：该音符发出一个新的歌词字/词（内部值 2）。
- **延长上字**：一字多音，继续唱紧邻的上一个字（内部值 3）。
- **休止**：此时段不唱（内部值 1，等同 `<SP>`）。

音高可填写 MIDI 1–127 或音名（例如 `C4`、`F#4`），MIDI 0 保留给休止（用“休止”唱法表示）。修改片段歌词后，可用“把上方歌词自动铺到音符”保留原音高、原总时长和休止位置，然后先检查表格再保存。复杂滑音、密集一字多音仍建议导出 MIDI 到 DAW / MIDI 编辑器精修。

导入 metadata JSON 或 MIDI 时，界面会先显示片段、音符、歌词起音、休止和主唱轨摘要。metadata JSON 直接提供逐字歌词、音高与时长；MIDI 更适合外部编辑，但多音符轨必须选择主唱轨。片段导出文件以该片段起点为 0 秒，不能作为整曲绝对时间文件导回。
